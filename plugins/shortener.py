...
async def URIShortener(update, context):
    keyboard = [
        [
    ...
    await update.message.reply_text("""Choose The Service:""", reply_markup=ikeyboard, reply_to_message_id=update.message.message_id)


async def cbackResponse(update, context):
    ...
    try:
        if cbdata.startswith("tiny"):
            ...
            await query.edit_message_text(sh_uri)
